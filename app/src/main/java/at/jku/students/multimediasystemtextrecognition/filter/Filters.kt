...
import com.multimedia.imagefilters.IFilter
    ...
    class FilterFactory {
        fun createFilter(filterType: FilterTypes): IFilter {
            return when (filterType) {
                FilterTypes.BINARY -> BinaryFilter()
                FilterTypes.CONTRAST -> ContrastFilter()
                FilterTypes.MEDIAN -> MedianFilter()
                FilterTypes.AVERAGING -> AveragingFilter()
                FilterTypes.BLACK_WHITE -> BlackWhiteFilter()
                FilterTypes.SHARPEN -> SharpenFilter()
                FilterTypes.BRIGHTNESS_HSV -> BrightnessHSVFilter()
                FilterTypes.EDGE_COLORING -> EdgeColoringFilter()
                FilterTypes.SATURATION_HSV -> SaturationFromHSVFilter()
                FilterTypes.HUE_HSV -> HueFromHSVFilter()
            }
        }
    }
